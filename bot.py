...
BOT_TOKEN = "7819237318:AAHuPL5dK1el7X3R9s411RnZnr3sKu7uTZ0"
bot = telebot.TeleBot(BOT_TOKEN)

# Bot Admins: Add initial Telegram user IDs of admins
BOT_ADMINS = [5270316487, 7110762565]  # Replace with the actual Telegram user IDs of initial bot admins
...
# To manage the stop functionality
running_tasks = {}
...
def start_handler(message):
    bot.reply_to(
        message,
        "Welcome to the Last Warning Bot!\n"
        "Commands:\n"
        "/GALIRAID <username> - Send GALIRAID messages non-stop (Admin only)\n"
        "/LOVERAID <username> - Send LOVERAID messages non-stop (Admin only)\n"
        "/stop - Stop sending messages (Admin only)\n"
        "/admin <user_id> - Add a new admin (Admin only)"
        "DEVELOPER : DARKBOY > @Darkboy336"
    )
...
def add_admin_handler(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ Please provide a valid user ID. Example: /admin 6829567767")

# Send GALIRAID messages non-stop to a specific user
@bot.message_handler(commands=['GALIRAID'])
def galiraid_handler(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ Please provide a username. Example: /GALIRAID @username")
        return

    if chat_id in running_tasks:
        bot.reply_to(message, "Messages are already being sent. Use /stop to end.")
        return

    bot.reply_to(message, f"Starting to send GALIRAID messages to {target_username} non-stop...")
    running_tasks[chat_id] = True

    def send_galiraid():
        while running_tasks.get(chat_id):
            random_msg = random.choice(GALIRAID)
            bot.send_message(chat_id, f"{target_username} {random_msg}")
            time.sleep(1)  # Adjust delay as needed
        running_tasks.pop(chat_id, None)

    threading.Thread(target=send_galiraid).start()

# Send LOVERAID messages non-stop to a specific user
@bot.message_handler(commands=['LOVERAID'])
def loveraid_handler(message):
    chat_id = message.chat.id
    user_id = message.from_user.id
    ...
    try:
        ...
        bot.reply_to(message, "❌ Please provide a username. Example: /LOVERAID @username")
        return
    ...
    bot.reply_to(message, f"Starting to send LOVERAID messages to {target_username} non-stop...")
    running_tasks[chat_id] = True

    def send_loveraid():
        while running_tasks.get(chat_id):
            random_msg = random.choice(LOVERAID)
            bot.send_message(chat_id, f"{target_username} {random_msg}")
            time.sleep(1)  # Adjust delay as needed
        running_tasks.pop(chat_id, None)

    threading.Thread(target=send_loveraid).start()
...
def stop_handler(message):
    ...
    if chat_id in running_tasks:
        ...
        bot.reply_to(message, "No messages are currently being sent.")
